...
import * as Engine from '../mini68/splendor-engine.js';
import * as AI from '../mini68/splendor-ai.js';

const MAX_MSG_LEN = 4000;
const ROOM_IDLE_MS = 1000 * 60 * 60 * 6;

export class SplendorRoom extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.seatCount = 4;
    this.fillBots = true;
  ...
  broadcastLobby() {
    this.broadcastRaw({ type: 'lobby', seats: this.seats, status: this.started ? 'started' : 'waiting' });
  }

  buildSyncFor(seatId) {
    const g = JSON.parse(JSON.stringify(this.game));
    for (const p of g.players) {
      if (p.id !== seatId) {
        p.reserved = p.reserved.map(() => ({ hidden: true }));
      }
    }
    return g;
  }
  ...
  broadcastSync() {
    for (const seat of this.seats) {
      ...
      this.sendTo(seat.id, { type: 'sync', state: this.buildSyncFor(seat.id), seats: this.seats });
    }
  }

  log(message) {
    this.broadcastRaw({ type: 'log', message });
  }
  ...
  async fetch(request) {
    ...
    if (this.started) this.sendTo(seatId, { type: 'sync', state: this.buildSyncFor(seatId), seats: this.seats });
    this.broadcastLobby();
  ...
  async handleMessage(seatId, data) {
    ...
    switch (data.type) {
      case 'takeThree':
        if (!isMyTurn) return;
        ok = Engine.takeThreeDifferent(g, seatId, data.colors);
        break;
      case 'takeTwo':
        if (!isMyTurn) return;
        ok = Engine.takeTwoSame(g, seatId, data.color);
        break;
      case 'reserve':
        if (!isMyTurn) return;
        ok = Engine.reserveCard(g, seatId, { tier: data.tier, cardId: data.cardId, blind: data.blind });
        break;
      case 'purchase': {
        if (!isMyTurn) return;
        const res = Engine.purchaseCard(g, seatId, { source: data.source, cardId: data.cardId });
        ok = res.ok;
        break;
      }
      case 'discard':
        ok = Engine.applyDiscard(g, seatId, data.resources || {});
        break;
      case 'claimNoble':
        ok = Engine.claimNoble(g, seatId, data.nobleId);
        break;
      default:
        return;
    }
    ...
    if (g.turn === seatId && g.phase === 'main' && !g.pendingNobleChoice) {
      Engine.endTurn(g);
    }
  ...
  advance() {
    ...
    for (let guard = 0; guard < 200; guard++) {
      ...
      if (g.phase === 'discard') {
        ...
        for (const p of g.players) {
          if (p.isBot && g.pendingDiscard[p.id] != null) {
            Engine.applyDiscard(g, p.id, AI.chooseDiscard(g, p.id));
            resolvedBot = true;
          }
        }
        if (!resolvedBot) return; // waiting on a human's discard
      }

      if (g.pendingNobleChoice) {
        const p = g.players.find(x => x.id === g.pendingNobleChoice);
        if (p.isBot) {
          const nobleId = AI.chooseNoble(g, p.id);
          if (nobleId) Engine.claimNoble(g, p.id, nobleId);
        } else return; // waiting on a human's noble choice
      }

      const curSeat = this.seats.find(s => s.id === g.turn);
      if (!curSeat) return;

      if (g.phase === 'main' && !g.pendingNobleChoice) {
        if (curSeat.isBot) {
          AI.runBotTurn(g, g.turn, curSeat.botLevel);
          if (g.turn === curSeat.id && g.phase === 'main' && !g.pendingNobleChoice) {
            Engine.endTurn(g);
          }
          continue;
        }
  ...
  async webSocketClose(ws, code, reason, wasClean) {
    ...
    if (seatId) {
      ...
      await this.persist();
      this.broadcastRaw({ type: 'peer-left', seatId });
