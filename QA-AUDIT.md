...
### 설명문↔조작 전수 자동 대조 (qa-desc-audit.js)
- 결과: **설명이 약속한 조작이 코드에 없는 사례 0건** — 설명은 조작을 거짓 표기하지 않음.
- 조작 안내가 없던 게임 중 비직관 조작 30개를 정밀 확인 → mini4/16/21/84 등은
  버튼·입력창·탭으로 이미 모바일 플레이 가능(오탐).
- [x] mini92 골프: 조준이 화살표 전용이라 모바일에서 조준 불가 → 캔버스 좌/우 탭 조준 +
  가운데 탭 발사 추가 + 설명 보강.

## 최종 상태
- Playwright 전수 실행 검사: **100/100 정상 로드 (에러 0)**.
- 설명문↔조작 자동 대조: **불일치 0건**.
- 조작 방향(화살표 부호)·모바일 조작 수단: 전 게임 확보.
